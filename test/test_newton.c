...
static void test_newton_cubic(void **state) {
    (void)state; // Unused
    float root = root_newton(RootFinder_Cubic, Derivative_Cubic, 1.0, 1e-6);
...
static void test_newton_quadratic(void **state) {
    (void)state; // Unused
    float root = root_newton(RootFinder_Quadratic, Derivative_Quadratic,  1.0, 1e-6);
    assert_false(isnan(root));
...
static void test_newton_sin(void **state) {
    (void)state; // Unused
    float root = root_newton(RootFinder_Sine, Derivative_Sine,  0.5, 1e-6);
    assert_false(isnan(root));
...
static void test_newton_logarithm(void **state) {
    (void)state; // Unused
    float root = root_newton(RootFinder_Logarithmic, Derivative_Logarithmic,  1.0, 1e-6);
    assert_false(isnan(root));
...
static void test_newton_positive_quadratic(void **state) {
    (void)state; // Unused
    float root = root_newton(PositiveQuadratic, Derivative_PositiveQuadratic,  -2.0, 1e-6);
    assert_true(isnan(root));
}

static void test_newton_no_root(void **state) {
    (void)state; // Unused
    float root = root_newton(ConstantFunc, Derivative_ConstantFunc,  5.0, 1e-6);
    assert_true(isnan(root));
}